...
"""
####################################### Valeurs Initales Optimistes #####################################
"""


class Agent(object):

    def __init__(self, manchots, strategie, resultat_initiale=0, alpha=None):
        ...
        self.estimation = resultat_initiale * np.ones(self.z)
...
class Map(object):
    ...
    def plots(self, resultats, optimalite):
        ...
        plt.ylabel('Frequency of choosing the optimal initiative')
...
strategies = [
    Agent(manchots, Glouton(0), resultat_initiale=5, alpha=0.1),
    Agent(manchots, Glouton(0.1), alpha=0.1),
]

map = Map(manchots, strategies, 'Opitmistic Epsilon Glouton')
